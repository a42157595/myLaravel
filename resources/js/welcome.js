...
$(document).ready(function () {
    ...
    Vue.component('note-template', {
        props: ['post'],
        template: `
        <div class="card" :id="post.id" :data-type="post.type" v-bind:style="{backgroundColor: post.bgcolor}">
        ...
        <div class="footer">
            ...
            <div class="icon">
                <div class="changeColor myTooltip" data-toggle="tooltip" data-placement="bottom" title="變更顏色">
            ...
            <div class="icon">
                <div class="delete myTooltip" :data-type="post.type" data-toggle="tooltip" data-placement="bottom" title="刪除記事">
...
function labelOptionRead() {
    ...
    $(".close").click(function (e) {
        e.stopPropagation();
        content = $(this).closest(".labelOption").data('content');
        Swal.fire({
            title: '確定要刪除嗎?',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "delete",
                    url: `note/deleteLabel/${content}`,
                    dataType: "json",
                    success: function (r) {
                        if (r['status']) {
                            data = labelOptionData.find(function (item, Index, array) {
                                item.index = Index
                                return item.id == id;
                            });
                            labelOptionData.splice(data.index, 1);
                            icon = 'success';
                        } else
                            icon = 'warning';
                        Swal.fire({
...
function noteRead() {
    ...
    $(".color").click(function (e) {
        ...
        type = $(this).closest(".card").data('type');
        $.ajax({
            type: "put",
            url: `note/changeBgColor/${id}`,
            data: {
                _method: "put",
                color: color
            },
            ...
            success: function (r) {
                if (r['status']) {
                    if (type) {
                        fixedVueData.find(function (item, Index, array) {
                            if (item.id == id)
                                item.bgcolor = color;
                        });
                    } else {
                        otherVueData.find(function (item, Index, array) {
                            if (item.id == id)
                                item.bgcolor = color;
                        });
